...
% Calculate accuracy
% Convert predicted_classes back to label format (0 or 1)
predicted_labels = zeros(n, 1);
for i = 1:n
    [~, idx] = max(predicted_classes(i,:));
    predicted_labels(i) = idx - 1; % Convert back to 0,1 labels to match Y_labels
end

% Calculate and display accuracy
accuracy = sum(predicted_labels == Y_labels) / n * 100;
